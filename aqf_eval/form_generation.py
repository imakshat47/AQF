...
def operators_for_field(field: dict, operator_aware: bool = True) -> list[str]:
    ...
    ops = set()
    # Use observed non-null types, but always include null predicates when null_flavour observed.
    effective = set(dv_types)
    if primary: effective.add(primary)
    if any(t in effective for t in ["DV_CODED_TEXT"]): ops.update(["equals", "not_equals", "in", "contains"])
    if any(t in effective for t in ["DV_TEXT"]): ops.update(["contains", "equals"])
    ...
    if field.get("supports_null_flavour") or field.get("has_null_flavour") or "NULL_FLAVOUR" in effective:
        ops.update(["is_known", "is_unknown"])
    else:
        ops.update(["is_known", "is_unknown"])
    if not ops: ops.update(["is_known", "is_unknown"])
...
def generate_form(forest: dict, scores: dict, method: str = "aqf_full", kappa: int = 60, theta: float = 0.10, operator_aware: bool = True, seed: int = 42) -> dict:
    ...
    for f in selected:
        fields.append({
            "field_id": f["field_id"], "label": f.get("label"), "canonical_path": f.get("canonical_path"),
            "record_family": f.get("record_family"), "form_group": f.get("form_group"), "nested_subgroup": f.get("nested_subgroup"),
            "kind": f.get("kind"), "dv_type": f.get("dv_type"), "primary_dv_type": f.get("primary_dv_type"),
            "observed_dv_types": f.get("observed_dv_types"), "supports_null_flavour": f.get("supports_null_flavour"),
            "operators": operators_for_field(f, operator_aware=operator_aware), "score": f.get("score", 0.0),
        })
