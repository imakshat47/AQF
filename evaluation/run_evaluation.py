...
PARSER_VERSION="orbda_parser_v2_1_matcher_baselines"
...
def run(args):
    ...
    if cache_ok:
        ...
        forest=load_json(cache_dir/"canonical_forest.json"); scores=load_json(cache_dir/"queriability_scores.json"); record_count=forest.get("record_count",0)
    else:
        ...
        write_json(forest, cache_dir/"canonical_forest.json"); write_json(scores, cache_dir/"queriability_scores.json"); record_count=len(record_units)
    queries=load_benchmarks(args.benchmarks, include_cross=args.include_cross); useful=useful_field_labels(queries)
    methods=[("aqf_full", True, args.theta), ("flattened_topk", True, args.theta), ("frequency_only", True, args.theta), ("no_pruning", True, 0.0), ("aqf_topk_no_threshold", True, 0.0), ("no_operator_awareness", False, args.theta)]
    ...
    write_csv(audit_field_selection(forest, scores, forms, queries), out_dir/"field_selection_audit.csv")
    ...
    print(f"AQF v2.1 evaluation complete. Results: {out_dir}")
    print(f"Cache used: {cache_ok} | Queries: {len(queries)}")

def main():
    p=argparse.ArgumentParser(); p.add_argument("--data-dir", required=True); p.add_argument("--out-dir", default="results/aqf_eval_v2_1"); p.add_argument("--cache-dir", default=None); p.add_argument("--use-cache", action="store_true")
    ...
    p.add_argument("--include-cross", action="store_true"); p.add_argument("--kappa", type=int, default=30); p.add_argument("--theta", type=float, default=0.10); p.add_argument("--alpha", type=float, default=0.70); p.add_argument("--beta", type=float, default=0.30); p.add_argument("--lamb", type=float, default=0.25); p.add_argument("--random-trials", type=int, default=30); p.add_argument("--seed", type=int, default=42); run(p.parse_args())
